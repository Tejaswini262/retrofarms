    ...
    return (
      <div className="bg-[#F7F1E5] min-h-screen flex items-center justify-center">
        <div className="text-center px-6">
          ...
          <Link
            to="/shop"
            className="inline-flex items-center bg-[#2B1D11] hover:bg-[#3A2818] text-[#F7F1E5] px-8 py-3 rounded-full transition-colors"
          >
            Shop the farm
          </Link>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-4">
            {cart.map((c) => (
              <div
                key={c.key}
                className="bg-white border border-[#E4D9C1] rounded-2xl p-4 flex gap-5 items-center"
              >
                ...
                <div className="flex items-center gap-2 border border-[#E4D9C1] rounded-full p-1">
                  <button
                    ...
                    className="w-8 h-8 flex items-center justify-center hover:bg-[#EFE4CB] rounded-full text-[#2B1D11]"
                  >
                    <Minus size={14} />
                  </button>
                ...
                <div className="font-serif text-xl text-[#2B1D11] w-20 text-right">
                  ₹{c.price * c.qty}
                </div>
                <button
                  onClick={() => removeFromCart(c.key)}
                  className="text-[#7A6A55] hover:text-red-600 transition-colors"
                >
                  <Trash2 size={18} />
                </button>
              </div>
          ...
          <div className="bg-white border border-[#E4D9C1] rounded-2xl p-8 h-fit">
            ...
            <div className="flex justify-between text-[#4B3826] mb-3">
              <span>Delivery</span>
              <span className="text-[#4E6A3C]">Free</span>
            </div>
            <div className="h-px bg-[#E4D9C1] my-4" />
            <div className="flex justify-between font-serif text-xl text-[#2B1D11] mb-6">
              <span>Total</span>
              <span>₹{cartTotal}</span>
            </div>
            <button
              onClick={() => {
                ...
                else alert('Checkout is mocked in this preview.');
              }}
              className="w-full bg-[#2B1D11] hover:bg-[#3A2818] text-[#F7F1E5] rounded-full py-3 transition-colors"
            >
              {user ? 'Proceed to checkout' : 'Sign in to checkout'}
            </button>
            <div className="text-xs text-[#7A6A55] mt-4 text-center">
              COD & Razorpay supported (mocked in preview)
            </div>
          </div>
        </div>
      </div>
